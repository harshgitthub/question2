...
# 4) Complete the SyncService implementation. Note that the SyncService.onMessage and SyncService.init function signature must not be altered.
...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        if data['type'] == 'probe':
            dev_id = data['dev_id']
            start_from = data['from']
            response_data = self.records[start_from:]
            return {'type': 'update', 'from': start_from, 'data': response_data}
...
def testSyncing():
    ...
    for i,rec in enumerate(devices[0].records):
        dev_idx = int(rec['dev_id'].split("_")[-1])
        assertEquivalent(rec, devices[dev_idx].sent[ver_start[dev_idx]])
